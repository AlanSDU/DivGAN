...
parser.add_argument('--image_size_l', dest='image_size_l', type=int, default=256, help='size of input images (applicable to both A images and B images)')
parser.add_argument('--image_size_w', dest='image_size_w', type=int, default=256, help='size of input images (applicable to both A images and B images)')
...
parser.add_argument('--lambda_Sim_loss', dest='lambda_Sim_loss', type=float, default=20.0, help='# weights of analogy loss')
parser.add_argument('--suffix', dest='suffix', default='jpg', help='jpg, png, etc.')
...
def main(_):
    ...
    with tf.Session() as sess:
        model = DualNet(sess, image_size_l=args.image_size_l, image_size_w=args.image_size_w, batch_size=args.batch_size,\
                        dataset_name=args.dataset_name,A_channels = args.A_channels, \
						B_channels = args.B_channels, flip  = (args.flip == 'True'),\
                        checkpoint_dir=args.checkpoint_dir, sample_dir=args.sample_dir,\
						fcn_filter_dim = args.fcn_filter_dim,ngf = args.ngf, use_res=args.use_resnet, \
						loss_metric=args.loss_metric, lambda_B=args.lambda_B, suffix=args.suffix, \
						lambda_A= args.lambda_A, lambda_Sim_loss= args.lambda_Sim_loss)
